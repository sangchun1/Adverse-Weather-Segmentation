#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Tune SegFormer hyperparameters with Optuna.

Main-branch version.

This script is a generalized version of the rain-specific tuning script.
It is intended to tune the model axis only, while using the current best
loss / augmentation / enhancement settings from a base config such as
...
Examples:
    ...
    python scripts/tune_segformer.py \
      --resume-study \
      --storage sqlite:///outputs/tuning/segformer/optuna.db
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Tune SegFormer hyperparameters with Optuna."
    )
    parser.add_argument(
        "--config",
        type=str,
        default="configs/proposed.yaml",
        help="Base config. This should already contain the best loss/augmentation/enhancement.",
    )
    parser.add_argument(
        "--condition",
        type=str,
    ...
    parser.add_argument(
        "--resume-study",
        action="store_true",
        help="Reuse an existing Optuna study when storage already exists.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory for tuning artifacts. Defaults to tuning.output_dir or outputs/tuning/segformer.",
    )
    parser.add_argument(
        "--device",
        type=str,
        default="",
        help="Optional device argument passed to awseg.train/evaluate if supported, e.g. cuda:0.",
    )
    parser.add_argument(
        "--run-eval",
        action="store_true",
...
def apply_trial_config(
    ...
    # Force SegFormer for this tuning script.
    ...
    if "classifier_dropout_prob" in search_space and "dropout" not in search_space:
        model_config["dropout"] = float(
            suggest_categorical(
                ...
                search_space["classifier_dropout_prob"],
            )
        )
    ...
    if "encoder_lr_mult" in search_space:
        optimizer_config["encoder_lr_mult"] = float(
            suggest_categorical(trial, "encoder_lr_mult", search_space["encoder_lr_mult"])
        )
    ...
    # Optional extra spaces if config explicitly defines them.
    if "lr" in search_space:
        optimizer_config["lr"] = float(suggest_categorical(trial, "lr", search_space["lr"]))
    ...
    experiment_config["group"] = "model_tuning"
    ...
    if disable_wandb or bool(config.get("tuning", {}).get("disable_wandb", True)):
        ...
        wandb_config["run_name"] = f"tune_segformer_{run_name}"
        tags = list(wandb_config.get("tags", []) or [])
        for tag in ["tuning", "segformer", run_name]:
            if tag not in tags:
                tags.append(tag)
...
def main() -> None:
    ...
    output_dir = Path(
        args.output_dir
        or tuning_config.get("output_dir", "outputs/tuning/segformer")
    )
    ...
    n_trials = int(args.n_trials or tuning_config.get("n_trials", 20))
    max_epochs_per_trial = args.max_epochs_per_trial
    if max_epochs_per_trial is None:
        max_epochs_per_trial = tuning_config.get("max_epochs_per_trial", None)
    if max_epochs_per_trial is not None:
        max_epochs_per_trial = int(max_epochs_per_trial)

    study_name = str(args.study_name or tuning_config.get("study_name", "segformer_optuna"))
    storage = str(
        args.storage
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        checkpoint_name = trial_config.get("checkpoint", {}).get("save_best_name", "best_miou.pth")
